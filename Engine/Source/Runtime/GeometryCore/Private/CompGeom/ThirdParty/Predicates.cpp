...
// random functions commented out; we don't use them -JimmyA
//
///*****************************************************************************/
///*                                                                           */
...
int expansion_sum(int elen, REAL *e, int flen, REAL *f, REAL *h) /* e and h can be the same, but f and h cannot. */
{
  REAL Q;
  INEXACT REAL Qnew;
...
int expansion_sum_zeroelim1(int elen, REAL *e, int flen, REAL *f, REAL *h) /* e and h can be the same, but f and h cannot. */
{
  REAL Q;
  INEXACT REAL Qnew;
...
int expansion_sum_zeroelim2(int elen, REAL *e, int flen, REAL *f, REAL *h) /* e and h can be the same, but f and h cannot. */
{
  REAL Q, hh;
  INEXACT REAL Qnew;
...
REAL incircleadapt(REAL *pa, REAL *pb, REAL *pc, REAL *pd, REAL permanent)
{
  ...
  int temp8len, temp16alen = 0, temp16blen, temp16clen;
...
// exact predicate versions of insphere are ifdef'd out because their stack size
// upsets static analysis, and no one is using it for now
//  TODO: if you need insphere, disable the static analysis warning and remove the ifdef
//  please don't put anything on the heap, despite what the static analysis warning suggests
#ifdef SUPPORT_EXACT_INSPHERE
...
#endif //SUPPORT_EXACT_INSPHERE

// clear the massive amount of defines to protect sanity of unity builds
#undef INEXACT
#undef REAL
#undef REALPRINT
#undef REALRAND
