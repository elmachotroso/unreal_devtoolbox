		...
		for (uint16 Channel : SupportedChannels)
		{
			...
			if (FDatasmithMaxMatHelper::GetMaterialClass(Material) == EDSMaterialType::MultiMat)
			{
				...
				if ( Mtl* SubMaterial = Material->GetSubMtl(Channel % Material->NumSubMtls()) )
				{
					...
					{
						MeshElement->SetMaterial(SubMaterial->GetName().data(), DisplayedChannelID);
					}
				}
			}
			...
			{
				MeshElement->SetMaterial(Material->GetName().data(), DisplayedChannelID);
			}
		}
	}
}

TSharedPtr< IDatasmithMeshElement > FDatasmithMaxMeshExporter::ExportMesh(FMaxExportMeshArgs& ExportMeshArgs, TSet<uint16>& SupportedChannels, FString& Error)
{
	TSharedPtr< IDatasmithMeshElement > MeshElement;
	UnitMultiplier = ExportMeshArgs.UnitMultiplier;

	if (ExportMeshArgs.Node == NULL)
	{
		return MeshElement;
	}
	...
	if (ExportMeshArgs.DatasmithAttributes)
	{
		if (ExportMeshArgs.DatasmithAttributes->GetExportMode() == EStaticMeshExportMode::BoundingBox)
		{
			if (FillDatasmithMeshFromBoundingBox(DSMesh, ExportMeshArgs.Node, ExportMeshArgs.Pivot))
			{
				FDatasmithMeshExporter DatasmithMeshExporter;
				return DatasmithMeshExporter.ExportToUObject(ExportMeshArgs.ExportPath, ExportMeshArgs.ExportName, DSMesh, nullptr, FDatasmithExportOptions::LightmapUV);
			}
			else
			{
				return MeshElement;
	...
	if (!CreateDatasmithMesh(DSMesh, ExportMeshArgs, SupportedChannels))
	{
		return MeshElement;
	}

	TUniquePtr< FDatasmithMesh > DSCollisionMesh;
	bool bIsCollisionFromDatasmithAttributes;
	INode* CollisionNode = GetCollisionNode(ExportMeshArgs.Node, ExportMeshArgs.DatasmithAttributes, bIsCollisionFromDatasmithAttributes);
	if (CollisionNode)
	{

		FMaxExportMeshArgs CollisionExportArgs;
		CollisionExportArgs.Node = CollisionNode;
		CollisionExportArgs.bForceSingleMat = ExportMeshArgs.bForceSingleMat;
		CollisionExportArgs.UnitMultiplier = ExportMeshArgs.UnitMultiplier;
		TSet<uint16> CollisionSupportedChannels;

		DSCollisionMesh = MakeUnique< FDatasmithMesh >();
		FTransform ColliderPivot = FDatasmithMaxSceneExporter::GetPivotTransform(CollisionNode, ExportMeshArgs.UnitMultiplier);

		if (bIsCollisionFromDatasmithAttributes)
		{
			if (!ExportMeshArgs.bBakePivot)
			{
				FTransform RealPivot = FDatasmithMaxSceneExporter::GetPivotTransform(ExportMeshArgs.Node, ExportMeshArgs.UnitMultiplier);
				ColliderPivot = ColliderPivot * RealPivot.Inverse();
			}
			CollisionExportArgs.Pivot = ColliderPivot;
		}
		else
		{

			TimeValue Now = GetCOREInterface()->GetTime();
			FTransform NodeWTM = FTransformFromMatrix3(ExportMeshArgs.Node->GetNodeTM(Now), ExportMeshArgs.UnitMultiplier);
			FTransform ColliderNodeWTM = FTransformFromMatrix3(CollisionNode->GetNodeTM(Now), ExportMeshArgs.UnitMultiplier);
			...
			CollisionExportArgs.Pivot = BakedTransform;
		}

		CreateDatasmithMesh(*DSCollisionMesh, CollisionExportArgs, CollisionSupportedChannels);
	}

	FDatasmithMeshExporter DatasmithMeshExporter;
	MeshElement = DatasmithMeshExporter.ExportToUObject(ExportMeshArgs.ExportPath, ExportMeshArgs.ExportName, DSMesh, DSCollisionMesh.Get(), FDatasmithExportOptions::LightmapUV );

	if ( !MeshElement.IsValid() )
	{
		Error = DatasmithMeshExporter.GetLastError();
	}
	else
	{
		AssignMeshMaterials(MeshElement, ExportMeshArgs.Node->GetMtl(), SupportedChannels);

		if (ExportMeshArgs.ExportName)
		{
			// Mapping between the 3ds max channel and the exported mesh channel
			const FUVChannelsMap* UVChannelMap = MeshNamesToUVChannels.Find(ExportMeshArgs.ExportName);
			if (ExportMeshArgs.DatasmithAttributes && UVChannelMap)
			{
				constexpr int32 MaxToUnrealUVOffset = -1;
				constexpr int32 DefaultValue = -1;
				const int32 SelectedLightmapUVChannel = ExportMeshArgs.DatasmithAttributes->GetLightmapUVChannel();
				const int32* ExportedSelectedChannel = UVChannelMap->Find(SelectedLightmapUVChannel + MaxToUnrealUVOffset);

				if (ExportedSelectedChannel)
				{
					MeshElement->SetLightmapCoordinateIndex(*ExportedSelectedChannel);
				}
				else if (SelectedLightmapUVChannel != DefaultValue)
				{
					DatasmithMaxLogger::Get().AddGeneralError(*FString::Printf(TEXT("%s won't use the channel %i for its lightmap because it's not supported by the mesh. A new channel will be generated.")
						, static_cast<const TCHAR*>(ExportMeshArgs.Node->GetName())
